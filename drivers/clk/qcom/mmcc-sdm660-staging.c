...
static const struct freq_tbl ftbl_video_core_clk_src[] = {
	...
	F(320000000, P_MMPLL7_PLL_OUT_MAIN, 3, 0, 0),
	F(404000000, P_MMPLL0_PLL_OUT_MAIN, 2, 0, 0),
	F(441600000, P_MMPLL3_PLL_OUT_MAIN, 2, 0, 0),
	F(518400000, P_MMPLL3_PLL_OUT_MAIN, 2, 0, 0),
	{ }
};
...
static struct clk_regmap *mmcc_sdm660_clocks[] = {
...
static const struct qcom_reset_map mmcc_sdm660_resets[] = {
	[CAMSS_MICRO_BCR] = { 0x3490 },
};

static const struct regmap_config mmcc_sdm660_regmap_config = {
	.reg_bits	= 32,
	.reg_stride	= 4,
	.val_bits	= 32,
...
static const struct qcom_cc_desc mmcc_sdm660_desc = {
	.config = &mmcc_sdm660_regmap_config,
	.clks = mmcc_sdm660_clocks,
	.num_clks = ARRAY_SIZE(mmcc_sdm660_clocks),
	.resets = mmcc_sdm660_resets,
	.num_resets = ARRAY_SIZE(mmcc_sdm660_resets),
	//.clk_hws = mmcc_sdm660_hws,
	//.num_clk_hws = ARRAY_SIZE(mmcc_sdm660_hws),
};

static const struct of_device_id mmcc_sdm660_match_table[] = {
	{ .compatible = "qcom,mmcc-sdm660" },
	{ .compatible = "qcom,mmcc-sdm630" },
	{ }
};
MODULE_DEVICE_TABLE(of, mmcc_sdm660_match_table);

static int mmcc_sdm660_probe(struct platform_device *pdev)
{
	struct regmap *regmap;
	bool is_sdm630 = 0;

	regmap = qcom_cc_map(pdev, &mmcc_sdm660_desc);
	if (IS_ERR(regmap))
	...
	if (is_sdm630) {
		mmcc_sdm660_desc.clks[BYTE1_CLK_SRC] = 0;
		mmcc_sdm660_desc.clks[MMSS_MDSS_BYTE1_CLK] = 0;
		mmcc_sdm660_desc.clks[MMSS_MDSS_BYTE1_INTF_DIV_CLK] = 0;
		mmcc_sdm660_desc.clks[MMSS_MDSS_BYTE1_INTF_CLK] = 0;
		mmcc_sdm660_desc.clks[ESC1_CLK_SRC] = 0;
		mmcc_sdm660_desc.clks[MMSS_MDSS_ESC1_CLK] = 0;
		mmcc_sdm660_desc.clks[PCLK1_CLK_SRC] = 0;
		mmcc_sdm660_desc.clks[MMSS_MDSS_PCLK1_CLK] = 0;
	}

	return qcom_cc_really_probe(pdev, &mmcc_sdm660_desc, regmap);
}

static struct platform_driver mmcc_sdm660_driver = {
	.probe		= mmcc_sdm660_probe,
	.driver		= {
		.name	= "mmcc-sdm660",
		.of_match_table = mmcc_sdm660_match_table,
	},
};

static int __init mmcc_sdm660_init(void)
{
	return platform_driver_register(&mmcc_sdm660_driver);
}
core_initcall(mmcc_sdm660_init);

static void __exit mmcc_sdm660_exit(void)
{
	platform_driver_unregister(&mmcc_sdm660_driver);
}
module_exit(mmcc_sdm660_exit);
