...
enum ipa_status_opcode {				/* *Not* a bitmask */
	...
	IPA_STATUS_OPCODE_SUSPENDED_PACKET	= 3,
        IPA_STATUS_OPCODE_XLAT_PACKET		= 6,
};
...
static bool ipa_endpoint_data_valid_one(struct ipa *ipa, u32 count,
			    const struct ipa_dma_endpoint_data *all_data,
			    const struct ipa_dma_endpoint_data *data)
{
	const struct ipa_dma_endpoint_data *other_data;
...
static u32 ipa_endpoint_max(struct ipa *ipa, u32 count,
			    const struct ipa_dma_endpoint_data *data)
{
	const struct ipa_dma_endpoint_data *dp = data;
	struct device *dev = &ipa->pdev->dev;
...
static struct ipa_dma_trans *ipa_endpoint_trans_alloc(struct ipa_endpoint *endpoint,
						  u32 tre_count)
{
	struct ipa_dma *ipa_dma = &endpoint->ipa->ipa_dma;
...
int ipa_endpoint_modem_exception_reset_all(struct ipa *ipa)
{
	...
	for_each_set_bit(endpoint_id, ipa->defined, ipa->endpoint_count) {
		 ...
		 */
		ipa_cmd_register_write_add(trans, offset, value, mask, false);
	}
...
static void ipa_endpoint_init_cfg(struct ipa_endpoint *endpoint)
{
	...
	if (endpoint->config.checksum) {
		if (endpoint->toward_ipa) {
			...
			enabled = IPA_CS_OFFLOAD_UL;
		} else {
			enabled = IPA_CS_OFFLOAD_DL;
		}
	} else {
...
int ipa_endpoint_skb_tx(struct ipa_endpoint *endpoint, struct sk_buff *skb)
{
	...
	ipa_dma = trans->ipa_dma;
	ipa_dma->ops->trans_commit(trans, !netdev_xmit_more());
...
static void ipa_endpoint_replenish(struct ipa_endpoint *endpoint)
{
	...
	while ((trans = ipa_endpoint_trans_alloc(endpoint, 1))) {
		...
		ipa_dma = trans->ipa_dma;
		ipa_dma->ops->trans_commit(trans, doorbell);
	}
...
static bool ipa_status_format_packet(enum ipa_status_opcode opcode)
{
	...
	case IPA_STATUS_OPCODE_XLAT_PACKET:
		return true;
	default:
		return false;
	}
...
void ipa_endpoint_suspend_one(struct ipa_endpoint *endpoint)
{
	...
	ret = ipa_dma->ops->channel_suspend(ipa_dma, endpoint->channel_id);
	if (ret)
...
void ipa_endpoint_resume_one(struct ipa_endpoint *endpoint)
{
	...
	ret = ipa_dma->ops->channel_resume(ipa_dma, endpoint->channel_id);
	if (ret)
		      ...
		      const struct ipa_dma_endpoint_data *data)
{
	...
	for (name = 0; name < count; name++, data++) {
		if (data->ee_id == IPA_EE_AP && !data->channel.tlv_count)
